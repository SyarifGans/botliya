...
prefix = "!"
name = "LIYA BOT"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN LIYA-BOT"
rmenu = "HAI TEMEN LIYA👋 JANGAN LUPA DONASI YAA:)"
botinfo = "UNTUK INVITE BOT SILAHKAN DONASI DULU YAA:)"
limitt = 20
memberLimit = 3
ban = []
premium = ["6281549391681@s.whatsapp.net"]
...
async function starts() {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6282351208767@s.whatsapp.net"] // owner number ubah aja
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		case 'daftar':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Temen Liya:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
